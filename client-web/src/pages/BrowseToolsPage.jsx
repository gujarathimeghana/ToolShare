    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Discover drills, saws, mowers, ladders, and party gear nearby.</p>
        </div>
        <button
          onClick={() => setShowMap(!showMap)}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-sm font-semibold text-slate-700 dark:text-slate-200 self-start"
        >
          <FiMapPin className="text-primary" /> {showMap ? 'Hide Proximity Map' : 'Show Proximity Map'}
        </button>
      </div>
      ...
      {showMap && (
        <div className="rounded-2xl overflow-hidden shadow-lg border border-slate-200 dark:border-slate-800">
          <OpenStreetMap markers={mapMarkers} height="350px" />
        </div>
      )}
      ...
      <div className="p-4 rounded-2xl glassmorphism border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm text-slate-800 dark:text-slate-200 font-medium focus:outline-none"
          >
